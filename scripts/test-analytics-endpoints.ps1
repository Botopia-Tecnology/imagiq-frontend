...
# Script para validar que los endpoints de analytics del backend están funcionando correctamente
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        if ($response.Content -match "GTM-MS5J6DQT") {
            ...
            Write-Host "   [FAIL] GTM Bootstrap FAILED: GTM ID not found" -ForegroundColor Red
        }
    } else {
        $errors += "GTM Bootstrap: Unexpected status $($response.StatusCode)"
        Write-Host "   [FAIL] GTM Bootstrap FAILED: Status $($response.StatusCode)" -ForegroundColor Red
    }
} catch {
    $errors += "GTM Bootstrap: $_"
    Write-Host "   [ERROR] GTM Bootstrap ERROR: $_" -ForegroundColor Red
}
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        if ($response.Content -match "25730530136536207") {
            ...
            $errors += "Meta Pixel Bootstrap: Pixel ID 25730530136536207 not found"
            Write-Host "   [FAIL] Meta Pixel Bootstrap FAILED: Pixel ID not found" -ForegroundColor Red
        }
    } else {
        $errors += "Meta Pixel Bootstrap: Unexpected status $($response.StatusCode)"
        Write-Host "   [FAIL] Meta Pixel Bootstrap FAILED: Status $($response.StatusCode)" -ForegroundColor Red
    }
} catch {
    $errors += "Meta Pixel Bootstrap: $_"
    Write-Host "   [ERROR] Meta Pixel Bootstrap ERROR: $_" -ForegroundColor Red
}
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        if ($response.Content -match "<iframe" -and $response.Content -match "GTM-MS5J6DQT") {
            ...
            Write-Host "      - Status: $($response.StatusCode)" -ForegroundColor Gray
            Write-Host "      - Contains iframe: Yes" -ForegroundColor Gray
        } else {
            $errors += "GTM Noscript: Invalid iframe or GTM ID missing"
            Write-Host "   [FAIL] GTM Noscript FAILED: Invalid HTML" -ForegroundColor Red
        }
    } else {
        $errors += "GTM Noscript: Unexpected status $($response.StatusCode)"
        Write-Host "   [FAIL] GTM Noscript FAILED: Status $($response.StatusCode)" -ForegroundColor Red
    }
} catch {
    $errors += "GTM Noscript: $_"
    Write-Host "   [ERROR] GTM Noscript ERROR: $_" -ForegroundColor Red
}
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        if ($response.Content -match "<img" -and $response.Content -match "25730530136536207") {
            ...
            Write-Host "      - Status: $($response.StatusCode)" -ForegroundColor Gray
            Write-Host "      - Contains img: Yes" -ForegroundColor Gray
        } else {
            $errors += "Meta Pixel Noscript: Invalid img tag or Pixel ID missing"
            Write-Host "   [FAIL] Meta Pixel Noscript FAILED: Invalid HTML" -ForegroundColor Red
        }
    } else {
        $errors += "Meta Pixel Noscript: Unexpected status $($response.StatusCode)"
        Write-Host "   [FAIL] Meta Pixel Noscript FAILED: Status $($response.StatusCode)" -ForegroundColor Red
    }
} catch {
    $errors += "Meta Pixel Noscript: $_"
    Write-Host "   [ERROR] Meta Pixel Noscript ERROR: $_" -ForegroundColor Red
}

Write-Host ""

# Test 5: Health Check (opcional)
...
try {
    ...
    if ($response.StatusCode -eq 200) {
        ...
        Write-Host "   [WARN] API Health: Unexpected status $($response.StatusCode)" -ForegroundColor Yellow
    }
} catch {
    $warnings += "API Health: No health endpoint found (this is optional)"
...
if ($errors.Count -eq 0) {
    Write-Host "SUCCESS: ALL TESTS PASSED ($($warnings.Count) warnings)" -ForegroundColor Green
    ...
    Write-Host "FAILED: $($errors.Count) TEST(S) FAILED" -ForegroundColor Red
    Write-Host ""
    ...
    Write-Host "1. Ensure backend is running in customer-success-ms" -ForegroundColor Gray
    Write-Host "2. Ensure api-gateway is running on port 3001" -ForegroundColor Gray
    Write-Host "3. Check CORS configuration in api-gateway/src/main.ts" -ForegroundColor Gray
    Write-Host "4. Verify .env variables in customer-success-ms" -ForegroundColor Gray
