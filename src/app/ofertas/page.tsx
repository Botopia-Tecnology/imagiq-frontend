    ...
    >
      ...
      >
        ...
        <div className="flex flex-col h-full md:hidden">
          <div className="flex flex-row h-1/2 gap-4 px-2 pt-38">
            {ofertas.slice(0, 2).map((item) => (
              <div
                key={item.title}
              ...
              >
                ...
                <Link
                  href={item.href}
                  className="mt-1 px-3 py-0.5 border border-white/80 rounded-full text-white text-xl font-medium hover:bg-white hover:text-[#1A407A] transition-colors duration-200 group-hover:bg-white group-hover:text-[#1A407A]"
          ...
          <div className="flex flex-row h-1/2 gap-3 px-2 pb-4">
            {ofertas.slice(2, 4).map((item) => (
              <div
                key={item.title}
                className="flex-1 flex flex-col items-center justify-center bg-white/10 border border-white/40 rounded-xl shadow-md backdrop-blur-md group transition-all duration-300 cursor-pointer h-full mx-1 px-2 py-2"
                style={{ height: "calc(30vh - 10px)", overflow: "hidden" }}
