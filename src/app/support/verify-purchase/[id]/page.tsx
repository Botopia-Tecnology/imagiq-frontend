...
export default function VerifySupportPurchase(props: Readonly<{ params: Readonly<Promise<{ id: string }>>; }>) {
  const { params } = props;
  ...
  const verifySupportOrder = useCallback(async () => {
    ...
    console.log("🔍 [VERIFY-SUPPORT] Iniciando verificación para orden de soporte:", orderId);

    try {
      const data = await apiGet(`/api/orders/support/verify/${orderId}`);
      console.log("📦 [VERIFY-SUPPORT] Response data completo:", JSON.stringify(data, null, 2));

      // Handle pending with additional action (if payments-ms returns such field)
      if (data.status === "PENDING" && data.requiresAction) {
        console.log("⏳ [VERIFY-SUPPORT] Transacción pendiente de validación. Reintentando en 5s...");
      ...
      const target = (s: string) => `/soporte/inicio_de_soporte?status=${encodeURIComponent(s)}&orderId=${encodeURIComponent(orderId)}`;

      if (data.status === 200 || data.status === "APPROVED") {
        console.log("✅ [VERIFY-SUPPORT] Pago aprobado, redirigiendo a soporte...");
        router.push(target("APPROVED"));
      } else if (data.status === "PENDING") {
        console.log("⏳ [VERIFY-SUPPORT] Pago pendiente. Redirigiendo a soporte para estado pendiente...");
      ...
      console.error("💥 [VERIFY-SUPPORT] Error verificando orden de soporte:", error);
