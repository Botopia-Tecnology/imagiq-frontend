...
export function HeroSection() {
  return (
    // Outer wrapper keeps white gutters like the Samsung design (centered max width)
    <div className="w-full flex justify-center">
      <div className="relative h-[520px] w-full max-w-[1400px] overflow-hidden flex items-center justify-center">
        {/* Background image constrained to the centered container so sides show white gutters */}
        <Image
        ...
        {/* Search Bar */}
        <div className="bg-white/95 backdrop-blur-md rounded-full p-2 flex items-center max-w-3xl mx-auto mb-6 border border-gray-200 shadow-lg">
          <svg
        ...
        {/* Quick Links */}
        <div className="flex flex-col gap-2 justify-center max-w-5xl mx-auto">
          {/* Primera fila - 3 botones */}
          <div className="flex gap-2 justify-center">
            {quickLinks.slice(0, 3).map((link, index) => (
              <button
                key={index}
                className="bg-white/10 backdrop-blur-sm hover:bg-white/20 text-white px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-200 shadow-sm border border-white/20"
              >
                {link}
              </button>
            ))}
